...
describe('Card', () => {
	...
	describe('click', () => {
		it('animates', async () => {
			await FPS();
			await page.goto(`http://${serverAddr}/#/card`);
			await page.waitForSelector('#card');
			await page.click('#card'); // to move mouse on the card.
	...
	it('should have a good CLS, FCP, INP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			const cardPage = targetEnv === 'TV' ? page : await newPageMultiple();
			await cardPage.emulateCPUThrottling(CPUThrottling);
			await cardPage.goto(`http://${serverAddr}/#/card`);
			await cardPage.addScriptTag({path: webVitalsPath});
			await new Promise(r => setTimeout(r, 100));

			const stepVitals = collectWebVitals(cardPage);

			await cardPage.evaluateHandle(() => {
			...
			await cardPage.waitForSelector('#card');
			await cardPage.focus('#card');
			await new Promise(r => setTimeout(r, 300));
			// Activate/select the card several times so a qualifying interaction lands
			...
			if (targetEnv === 'PC') await cardPage.close();
		}
