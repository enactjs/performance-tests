...
describe('ActionGuide', () => {
	const component = 'ActionGuide';
	TestResults.newFile(component);

	describe('click', () => {
		it('animates', async () => {
			await FPS();
			await page.goto(`http://${serverAddr}/#/actionGuide`);
			await page.waitForSelector('#actionGuide');
			await page.click('#actionGuide'); // to move mouse on the actionGuide.
	...
	it('should have a good CLS, FCP, INP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			const actionGuidePage = targetEnv === 'TV' ? page : await newPageMultiple();
			await actionGuidePage.emulateCPUThrottling(CPUThrottling);
			await actionGuidePage.goto(`http://${serverAddr}/#/actionGuide`);
			await actionGuidePage.addScriptTag({path: webVitalsPath});
			await new Promise(r => setTimeout(r, 100));

			const stepVitals = collectWebVitals(actionGuidePage);

			await actionGuidePage.evaluateHandle(() => {
			...
			await actionGuidePage.waitForSelector('#actionGuide');
			await actionGuidePage.focus('#actionGuide');
			await new Promise(r => setTimeout(r, 300));
			// Activate/select the actionGuide several times so a qualifying interaction lands
			// each step even before Spotlight/focus has settled on the first pages.
			await actionGuidePage.keyboard.press('Enter');
			for (let i = 0; i < 6; i++) {
				await actionGuidePage.keyboard.press('Enter');
			...
			if (targetEnv === 'PC') await actionGuidePage.close();
		}
