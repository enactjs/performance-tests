...
describe('Slider', () => {
	...
	it('should have a good CLS, FCP, INP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			...
			await sliderPage.addScriptTag({url: webVitalsURL});
			await new Promise(r => setTimeout(r, 100));
			await sliderPage.waitForSelector('#slider');
			await sliderPage.focus('#slider');
			await new Promise(r => setTimeout(r, 200));
			await sliderPage.keyboard.down('Enter');
			await sliderPage.keyboard.up('Enter');
			await new Promise(r => setTimeout(r, 200));
			...
			await sliderPage.evaluateHandle(() => {
