...
describe('PageViews', () => {
	const component = 'PageViews';
	TestResults.newFile(component);

	describe('click', () => {
		it('animates', async () => {
			await FPS();
			await page.goto(`http://${serverAddr}/#/pageViews`);
			await page.waitForSelector('#pageViews');
			await page.click('#pageViews'); // to move mouse on the pageViews.
	...
	it('should have a good CLS, FCP, INP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			const pageViewsPage = targetEnv === 'TV' ? page : await newPageMultiple();
			await pageViewsPage.emulateCPUThrottling(CPUThrottling);
			await pageViewsPage.goto(`http://${serverAddr}/#/pageViews`);
			await pageViewsPage.addScriptTag({path: webVitalsPath});
			await new Promise(r => setTimeout(r, 100));

			const stepVitals = collectWebVitals(pageViewsPage);

			await pageViewsPage.evaluateHandle(() => {
			...
			await pageViewsPage.waitForSelector('#pageViews');
			await pageViewsPage.focus('#pageViews');
			await new Promise(r => setTimeout(r, 300));
			// Activate/select the pageViews several times so a qualifying interaction lands
			// each step even before Spotlight/focus has settled on the first pages.
			await pageViewsPage.keyboard.press('ArrowRight');
			for (let i = 0; i < 6; i++) {
				await pageViewsPage.keyboard.press('ArrowRight');
			...
			if (targetEnv === 'PC') await pageViewsPage.close();
		}
