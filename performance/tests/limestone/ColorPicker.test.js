...
describe('ColorPicker', () => {
	...
	describe('click', () => {
		it('animates', async () => {
			...
			await page.click('#colorPicker'); // to move mouse on the colorPicker.
	...
	it('should have a good CLS, FCP, INP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			...
			await colorPickerPage.evaluateHandle(() => {
			...
			await colorPickerPage.waitForSelector('#colorPicker');
			await colorPickerPage.focus('#colorPicker');
			await new Promise(r => setTimeout(r, 300));
			// Activate/select the colorPicker several times so a qualifying interaction lands
			// each step even before Spotlight/focus has settled on the first pages.
			await colorPickerPage.keyboard.press('Enter');
			for (let i = 0; i < 6; i++) {
				await colorPickerPage.keyboard.press('Enter');
