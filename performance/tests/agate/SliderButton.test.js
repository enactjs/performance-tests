...
describe('SliderButton', () => {
	...
	it('should have a good CLS, FCP, INP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			const sliderButtonPage = targetEnv === 'TV' ? page : await newPageMultiple();
			...
			sliderButtonPage.on("console", (msg) => {
			...
			await sliderButtonPage.evaluateHandle(() => {
			...
			if (targetEnv === 'PC') await sliderButtonPage.close();
		}
