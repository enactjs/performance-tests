...
describe('CheckboxItem', () => {
	...
	it('should have a good CLS, FCP, INP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			...
			await checkboxItemPage.addScriptTag({url: webVitalsURL});
			await new Promise(r => setTimeout(r, 100));
			await checkboxItemPage.waitForSelector('#agate-checkboxItem');
			await checkboxItemPage.focus('#agate-checkboxItem');
			await new Promise(r => setTimeout(r, 200));
			await checkboxItemPage.keyboard.down('Enter');
			await checkboxItemPage.keyboard.up('Enter');
			await new Promise(r => setTimeout(r, 200));
			await checkboxItemPage.keyboard.down('Enter');
			await checkboxItemPage.keyboard.up('Enter');
			await new Promise(r => setTimeout(r, 200));

			checkboxItemPage.on("console", (msg) => {
				let jsonMsg = {};
			...
			await checkboxItemPage.evaluateHandle(() => {
