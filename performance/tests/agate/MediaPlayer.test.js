...
describe('MediaPlayer', () => {
	...
	it('should have a good CLS, FCP, INP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			const mediaPlayerPage = targetEnv === 'TV' ? page : await newPageMultiple();
			...
			mediaPlayerPage.on("console", (msg) => {
			...
			await mediaPlayerPage.evaluateHandle(() => {
			...
			if (targetEnv === 'PC') await mediaPlayerPage.close();
		}
