...
describe('LabeledIcon', () => {
	...
	it('should have a good CLS, FCP and LCP', async () => {
		...
		for (let step = 0; step < stepNumber; step++) {
			const labeledIconPage = targetEnv === 'TV' ? page : await newPageMultiple();
			await labeledIconPage.emulateCPUThrottling(CPUThrottling);
			await labeledIconPage.goto(`http://${serverAddr}/#/labeledIcon`);
			await labeledIconPage.addScriptTag({path: webVitalsPath});
			await new Promise(r => setTimeout(r, 100));
			await labeledIconPage.waitForSelector('#labeledIcon');
			await labeledIconPage.focus('#labeledIcon');
			await labeledIconPage.keyboard.down('Enter');
			await new Promise(r => setTimeout(r, 200));

			const stepVitals = collectWebVitals(labeledIconPage);

			await labeledIconPage.evaluateHandle(() => {
			...
			if (targetEnv === 'PC') await labeledIconPage.close();
		}
